...
<div style="margin:100px auto; padding: 8px; border: 1px solid gray; background: #EAEAEA; width: %upx">
  ...
  <div id="msg" style="font-family: Tahoma; font-size: 9pt;">正在处理...</div>
...
for ($i = 0; $i < $length; $i++) {
  ...
  $script = '<script>document.getElementById("percent").innerText="%u%%";document.getElementById("progress").style.width="%u%%";document.getElementById("msg").innerText="%s";</script>';
  // echo sprintf($script, intval($proportion * 100), intval(($i + 1) / $length) * $width, $msg);
  echo sprintf($script, intval($proportion * 100), intval($proportion * 100), $msg);
  echo ob_get_clean();    //获取当前缓冲区内容并清除当前的输出缓冲
  flush();   //刷新缓冲区的内容，输出
}
