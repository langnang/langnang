...
class db
{
  ...
  public static function insert($table = '', $data = null, $return_sql = false)
  {
    ...
    if ($return_sql) {
      ...
      if (self::query($sql)) {
        $insert_id = mysqli_insert_id(self::$links[self::$link_name]['conn']);
        if ($insert_id == 0) {
          log::debug("Warn Insert SQL: " . $sql);
        }
        return $insert_id;
      } else {
        log::error("Error Insert SQL: " . $sql);
        return false;
      }
    }
  }
  public static function upsert($table = '', $data = null, $uniqueColumns = null, $return_sql = false)
  {
    $uniqueColumns = array_intersect(array_keys($data), $uniqueColumns);
    if (empty($uniqueColumns)) {
      return self::insert($table, $data);
    } else {
      $sql = self::insert($table, $data, true) . " ON DUPLICATE KEY UPDATE";
      print_r($sql);
    ...
    if ($return_sql) {
      ...
      if (self::query($sql)) {
        $affected_rows = mysqli_affected_rows(self::$links[self::$link_name]['conn']);
        if ($affected_rows == 0) {
          log::debug("Warn Upsert SQL: " . $sql);
        }
        return $affected_rows;
      } else {
        log::error("Error Upsert SQL: " . $sql);
        return false;
      }
    }
  }
  ...
  public static function update($table = '', $data = array(), $where = null, $return_sql = false)
  {
    ...
    if ($return_sql) {
      ...
      if (self::query($sql)) {
        $affected_rows = mysqli_affected_rows(self::$links[self::$link_name]['conn']);
        if ($affected_rows == 0) {
          log::debug("Warn Update SQL: " . $sql);
        }
        return $affected_rows;
      } else {
        log::error("Error Update SQL: " . $sql);
        return false;
      }
    }
  }
  ...
  public static function select($table = '', $where = null, $suffix = '', $return_sql = false)
  {
    if (empty($where)) {
      return false;
    }
    $where = 'Where ' . (!is_array($where) ? $where : implode(' And ', $where));
    $sql = "SELECT * From `{$table}` {$where} {$suffix}";
  ...
  public static function select_exists($table = '', $where = null, $return_sql = false)
  {
    if (empty($where)) {
      return false;
    }
  ...
  public static function table_create($table_name, $columns, $return_sql = false) {}
  public static function table_delete($table_name, $columns, $return_sql = false) {}
  public static function table_update($table_name, $columns, $return_sql = false) {}

  public static function column_exists() {}
  public static function column_create() {}
  public static function column_delete() {}
  public static function column_update() {}
}
