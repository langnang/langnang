...
class phpspider
{
  ...
  public function __construct($configs = array())
  {
    ...
    self::$export_columns = isset($configs['export_columns'])  ? $configs['export_columns']  : [
      "primary" => array_values(array_map(function ($field) {
        return $field['name'];
      }, array_filter($configs['fields'], function ($field) {
        return !($field['primary_key'] ?? false);
      }))),
      "index" => array_values(array_map(function ($field) {
        return $field['name'];
      }, array_filter($configs['fields'], function ($field) {
        return !($field['data_index'] ?? false);
      }))),
      "update" => array_values(array_map(function ($field) {
        return $field['name'];
      }, array_filter($configs['fields'], function ($field) {
        return !(($field['data_index'] ?? false) || ($field['primary_key'] ?? false));
      }))),
    ];
  ...
  public function add_url($url, $options = array(), $depth = 0)
  {
    ...
    if ($this->is_content_page($url)) {
      ...
      // 如果 on_add_url 返回 false，此URL不入队列
      if ($url) {
  ...
  public function collect_page()
  {
    ...
    // 缓存html内容
    $parse_url = @parse_url(@substr($url, -1) == '/' ? $url . "index" : $url);
    // var_dump($parse_url);
    // exit;
    $storage_html_path = '/html/' . $parse_url['host'] . ($parse_url['path'] ?? '') . (isset($parse_url['query']) ? '/' . $parse_url['query'] : '');
    $html_path = storage_path($storage_html_path);
    // $html_path = '/../html/' . $parse_url['host'] . preg_replace("/\?/", "/", $parse_url['path'] ?? '');;
    if (file_exists($html_path)) {
      // $html = file_get_contents($html_path);
      $html = util::get_file($html_path);
      log::debug("Get storage file: " . $storage_html_path);
    } else {
      $html = $this->request_url($url, $link);
      if ($html) {
        util::put_file($html_path, $html) or log::error("Error Write Page: " . $url);
        log::debug("Put storage file: " . $storage_html_path);
      }
    }
    // if (file_exists(__DIR__ . '/..' . self::$configs['cache_path'])) {
    // } else {
    //   $html = $this->request_url($url, $link);
    // }
  ...
  {
    $url_parse = parse_url(strtolower($link['url']));
    $uri = '';
    ...
    $link = array(
      ...
      'uri'          => isset($link['uri'])          ? $link['uri']          : $uri,
      'path'         => isset($link['path'])         ? $link['path']         : $url_parse['path'] ?? '',
      'query'        => isset($link['query'])        ? $link['query']        : $url_parse['query'] ?? '',
      'url_parse'    => isset($link['url_parse'])    ? $link['url_parse']    : $url_parse,
      ...
      if (isset($fields) && is_array($fields)) {
        ...
        if (!empty(self::$configs['export'])) {
          ...
          if (self::$export_type == 'csv') {
            ...
            db::upsert(self::$export_table, $fields, self::$export_columns['update']);
            // exit;
          }
        }

        if (self::$configs['max_fields'] > 0 && $fields_num > self::$configs['max_fields'] - 1) {
          exit(0);
        }
      }
    }
  ...
  public function get_fields($confs, $html, $url, $page)
  {
    ...
    $this->set_fields_prop('config', self::$configs);
    $this->set_fields_prop('fields', $confs);
    $this->set_fields_prop('html',   $html);
    $this->set_fields_prop('url',    $url);
    $this->set_fields_prop('page',   $page);
    // var_dump($confs, $html, $url, $page);
    // var_dump($page);
    ...
    foreach ($confs as $conf) {
      ...
      $this->set_fields_prop('field',  $conf);

      $values = NULL;
      // 如果定义继承规则
      if (!empty($conf['prop'])) {

        $values = array_get(self::$fields_prop, $conf['prop']);

        if (!empty($conf['filter'])) $values = $this->filter_field($values, $conf['filter']);
      }

      // 如果定义抽取规则
      if (!isset($values) && !empty($conf['selector'])) {
        ...
        if (isset($conf['source_type']) && $conf['source_type'] == 'attached_url') {
          ...
          if (!empty($fields[$conf['attached_url']])) {
            ...
            if ($this->on_download_attached_page) {
              $return = call_user_func($this->on_download_attached_page, $html, $page, $this);
              if (isset($return)) {
                $html = $return;
              }
            }
  ...
  public function filter_field($values, $filters)
  {
    ...
    foreach ($filters as $filter) {
      ...
      if ($startTag && $endTag) {
        ...
        // var_dump($latestStr);
        $argStr = substr($filter, $startTag === false ? 0 :  $startTag + 1, ($endTag ?? 0) - $startTag - 1);
        // var_dump($argStr);
      }
      ...
      if (function_exists($func)) {
        // var_dump('filter function exists: ' . $func);
        // var_dump($args);
  ...
  public function set_fields_prop($key, $values)
  {
    self::$fields_prop = array_set(self::$fields_prop, $key, $values);
  }
  /**
   * 清空shell输出内容
   * 
  ...
  public function replace_echo($message, $force_clear_lines = NULL)
  {
    ...
    // 获取终端宽度
    $toss = $status = null;
    $term_width = util::is_win() ? null : exec('tput cols', $toss, $status);
  ...
  public function display_ui()
  {
    ...
    $export_config = self::$configs['export'] ?? [];
    $db_config = self::$configs['db_config'] ?? [];
    $queue_config = self::$configs['queue_config'] ?? [];
    ...
    if ($export_config['type'] == 'db') {
      // jdbc:mysql://localhost:3306/my_database?user=my_user&password=my_password
      $display_str .= "export config: mysql://{$db_config['host']}:{$db_config['port']}/{$db_config['name']}?table={$export_config['table']}" . "\n";
    }
    // redis://localhost:6379?timeout=3000&db=2&max_connections=50
    $display_str .= "queue config: redis://{$queue_config['host']}:{$queue_config['port']}?db={$queue_config['db']}" . "\n";
    $display_str .= "queue prefix: {$queue_config['prefix']}" . "\n";
