<?php

namespace App\Traits\Model;

/**
 * Summary of HasFamily
 * @method mixed parent()
 * @method mixed children()
 * @method mixed parents()
 * @method mixed child()
 */
trait HasFamily
{
    protected $prevKey = 'prev';
    public function getPrevKey()
    {
        return $this->prevKey;
    }
    protected $nextKey = 'next';
    public function getNextKey()
    {
        return $this->nextKey;
    }
    protected $parentKey = 'parent';
    public function getParentKey()
    {
        return $this->parentKey;
    }
    public function families() {}
    // 弟类
    public function next()
    {
        return $this->hasOne(static::class, $this->primaryKey, $this->nextKey)->with('next');
    }
    // 兄类
    public function prev()
    {
        return $this->hasOne(static::class, $this->primaryKey, $this->prevKey)->with('prev');
    }
    // 同级类
    public function sibling() {}
    /**
     * 父类
     */
    public function parent()
    {
        return $this->hasOne(static::class, $this->primaryKey, $this->parentKey)->with('parent');
    }

    public function root()
    {
        return $this->hasOne(static::class, $this->primaryKey, $this->parentKey,)->with('root');
    }
    /**
     * Retrieve the root parent of the current category.
     * The root parent of a category that has no parent is that category itself.
     *
     */
    public function getRoot()
    {
        $bubble_keys = [$this[$this->getKeyName()]];
        // $this->bubbule_keys = $this->active_keys ?? [$this[$this->getKeyName()]];
        if ($this->root) {
            if ($this->root->root) {
                $this->root = $this->root->getRoot();
                // dump($this->root->active_keys);
            }
            // array_unshift($bubble_keys);
            $this->root->bubble_keys = array_merge($this->root->bubble_keys ?? [$this->root[$this->getKeyName()]], $bubble_keys);
            return $this->root;
        }
        $this->bubble_keys = array_merge($this->bubble_keys ?? [], $bubble_keys);
        return $this;
        // if ($this->root && $this->root->root) {
        //   return $this->getRoot();
        // }
        // return $this->root;
    }
    /**
     * 子集
     */
    public function children()
    {
        return $this->hasMany(static::class, $this->parentKey, $this->primaryKey);
    }

    public function children_tree()
    {
        return $this->hasMany(static::class, $this->parentKey, $this->primaryKey)->with('children_tree');
    }
    public function getChildrenTreeIdsAttribute()
    {
        $return = $this->children_ids ?? [];
        // dump($this);
        // dump($this->children);
        foreach ($this->children as $child) {
            $child->append('children_ids');
            // dump($child->children_count);
            $return = array_merge($return, $child->children_ids);
            // $return += sizeof($child->children);
        }
        // dd(__METHOD__);
        return $this->attributes['children_count'] = $return;
    }

    public function getChildrenTreeCountAttribute()
    {
        $return = sizeof($this->children);
        // dump($this);
        // dump($this->children);
        foreach ($this->children as $child) {
            $child->append('children_count');
            // dump($child->children_count);
            $return += sizeof($child->children);
        }
        // dd(__METHOD__);
        return $this->attributes['children_count'] = $return;
    }
    public function family_modules()
    {
        return $this->children()->where([['type', 'module']]);
    }
    public function family_categories()
    {
        return $this->children()->where([['type', 'category']]);
    }
    public function family_tags()
    {
        return $this->children()->where([['type', 'tag']]);
    }
}
