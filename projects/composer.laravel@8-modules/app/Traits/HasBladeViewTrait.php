...
trait HasBladeViewTrait
{
    ...
    public function setBladeView($values, $key = null)
    {
        if (empty($key)) {
            $this->__bladeView = $values;
        } else {
            Arr::set($this->__bladeView, $key, $values);
        }
        return $this->__bladeView;
    }
    public function getBladeView($key = null)
    {
        return Arr::get($this->__bladeView, $key);
    }

    private function _initBladeView()
    {
        // user
        Arr::set($this->__bladeView, '_user', \Auth::user());
        // request
        Arr::set($this->__bladeView, '_request', request()->all());
        // route
        Arr::set($this->__bladeView, '_route', [
        ...
        Arr::set($this->__bladeView, '_server', $_SERVER);
        Arr::set($this->__bladeView, '_cookie', $_COOKIE);
        //Arr::set($this->__bladeView, '_session', $_SESSION);
        Arr::set($this->__bladeView, '_request', request()->all());
        Arr::set($this->__bladeView, '_md5', md5(json_encode([
            'url' => request()->url(),
            'fullUrl' => request()->fullUrl(),
        ], JSON_UNESCAPED_UNICODE)));
        Arr::set($this->__bladeView, '_plugins', dirsOf(public_path('plugins')));
    }

    public function mergeBladeView($values)
    {
        return $this->__bladeView = array_merge($this->__bladeView, $values);
    }
    /**
     * Summary of view
     * @param mixed $view
    ...
    {
        // $_return = array_merge([
        //     '_app' => $this->getBladeView(),
        //     '_user' => \Auth::user(),
        //     '_module' => $this->module,
        //     '_blade' => $this->getBladeView(),
        //     '_servers' => [],
        //     '_constants' => [],
        //     '_variables' => [
        //         'route' => [
        ...
        // $return = $this->getBladeView();
        // unset($return['entity']);
        // unset($return['_module']);
        // var_dump($return);
        // return $return;
        Arr::set($this->__bladeView, '_request', request()->all());
        Arr::set($this->__bladeView, '_time', date('Y-m-d h:i:s'));

        $return = array_merge($this->getBladeView(), is_array($view) ? $view : ['view' => $view], $data);
        // return $return;
        $return['layout'] = $this->matchLayout(
            Arr::get($return, 'layout'),
            Arr::get($return, 'framework'),
            Arr::get($return, 'alias')
        );
        $return['view'] = $this->matchView(
            Arr::get($return, 'view'),
            Arr::get($return, 'framework'),
            Arr::get($return, 'alias'),
        );
        ...
        // var_dump($return);
        // var_dump(json_encode($return, JSON_UNESCAPED_UNICODE));
        if (env('APP_DEBUG') || config('app.debug')) {
            echo '<script>console.log("/**\r\n * ToDos\r\n */\r\n- [ ] ' . implode('\r\n- [ ] ', Arr::get($return, '_module.todos', [])) . '");</script>';
     ...
     * 
     */
    private function matchLayout($layout = null, $framework = null, $alias = null)
    {
        // with alias
        if (View::exists($_full_layout = "$alias::layouts.$framework.$layout")) {
            return $_full_layout;
        } else if (View::exists($_full_framework = "$alias::layouts.$framework")) {
            return $_full_framework;
        } else if (View::exists($_framwork_layout = "$alias::layouts.$layout")) {
            return $_framwork_layout;
        }
        // without alias
        if (View::exists($_full_layout = "layouts.$framework.$layout")) {
            return $_full_layout;
        } else if (View::exists($_full_framework = "layouts.$framework")) {
            return $_full_framework;
        } else if (View::exists($_framwork_layout = "layouts.$layout")) {
            return $_framwork_layout;
        }
    }

    private function matchView($view, $framework = null, $alias = null)
    {
        Debugbar::info([
            __METHOD__,
            $view,
        ...
        if (View::exists($_full_view = "$alias::$framework.$view")) {
            return $_full_view;
        } else if (View::exists($_framwork_view = "$framework.$view")) {
            return $_framwork_view;
        } else {
            return $view;
        }
        $_framwork_view = "$framework.$view";
        if (empty($layout)) {
            $layout = $framework . '.' . $view;
            // var_dump($return['layout']);
            if (!View::exists($layout)) {
                $layout = $view;
            }
            // var_dump($data['layout']);
            $view = ($alias ?? '') . '::' . $framework . '.' . $view;
    ...
    //     return $this->__bladeView($return);
    // }
     ...
     */
    public function _view_index(Request $request)
    {
        // dump(Auth::check());
        ...
        return $this->__bladeView($return['view'], $return);
    }
    /**
    ...
    //     return $this->__bladeView_index($request);
    //     $return = array_merge([
    //         "prefix" => $this->prefix ?? 'home',
    //         "view" => \View::exists($this->prefix . '.meta') ? $this->prefix . '.meta' : '_view.meta',
    //     ], $request->input('$return', []),);
    //     $return = array_merge($this->__bladeView_index($request)->getData(), $return);
    //     echo "<script>window.\$data=" . json_encode($return, JSON_UNESCAPED_UNICODE) . "</script>";
    //     return $this->__bladeView($return['view'], $return);
    // }
     ...
     * @return \Illuminate\Contracts\Foundation\Application|\Illuminate\Contracts\View\Factory|\Illuminate\Contracts\View\View
     */
    public function _view_content_item(Request $request, $cid = 0)
    {
    ...
    public function view_spider_discover_item(Request $request, $spider_cid, $discover_index = 0)
    {
        ...
        // dump($paginator);
        // return $this->__bladeView('video.list', [
        //   "spider" => $spider,
        //   "discover_index" => $discover_index,
        //   "contents" => $paginator,
        // ]);
