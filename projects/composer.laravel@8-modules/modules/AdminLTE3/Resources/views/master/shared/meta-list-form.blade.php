          ...
          <div class="card-header d-flex align-items-center py-2">
            ...
            <div class="">
              ...
              <button type="button" class="btn btn-sm btn-warning"
                onclick="$('[name=_action]').val('factory');$('form[name=content-row]').submit()">Factory</button>
            </div>
          </div>
          <ul class="list-group list-group-flush">
            @foreach ($list ?? [] as $index => $item)
              <li class="list-group-item pt-1 pb-0 d-flex align-items-center">
                ...
                <div class="row">
                  <div class="col-md-4">
                    <input type="hidden" class="form-control" name='list[{{ $index }}][id]'
                      value="{{ $item['id'] }}">
                    <div class="form-group mb-1">
                      <div class="input-group input-group-sm">
                        ...
                        <input type="text" class="form-control" name='list[{{ $index }}][slug]'
                          value="{{ $item['slug'] }}">
                      </div>
                    </div>
                  </div>
                      ...
                      <div class="input-group input-group-sm">
                        ...
                        <select class="form-control" name='list[{{ $index }}][type]'>
                          @foreach (Arr::get($_module, 'options.meta.type', []) as $option)
                            <option value="{{ $option['value'] }}" @if ($option['value'] == $item['type']) selected @endif>
                              {{ $option['name'] }}</option>
                          @endforeach
                        </select>
                      </div>
                  ...
                  <div class="col-md-6">
                    <div class="form-group mb-1">
                      <div class="input-group input-group-sm">
                        ...
                        <input type="text" class="form-control" name='list[{{ $index }}][name]'
                          value="{{ $item['name'] }}">
                      </div>
                    </div>
                  </div>
                  ...
                  <div class="col-md-6">
                    <div class="form-group mb-1">
                      <div class="input-group input-group-sm">
                        ...
                        <input type="text" class="form-control" name='list[{{ $index }}][ico]'
                          value="{{ $item['ico'] }}">
                      </div>
                    </div>
                  </div>
                ...
                @foreach ($modules ?? [] as $meta_module)
                  ...
                  @foreach ($meta_module['children'] ?? [] as $meta_module_01)
                    <div class="form-check" style="padding-left: 1.5rem;">
                      <input class="form-check-input" type="radio" name="module"
                        value="{{ $meta_module_01['id'] }}">
