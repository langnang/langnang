...
@section('content')
  <div class="container py-3">
    <form class="card" method="POST">
      ...
      <div class="card-body">
        ...
        <div class="form-group row">
          ...
          <div class="col-sm-10">
            <input type="text" class="form-control" name="ico" value="{{ $link->ico }}">
          </div>
        </div>
        <div class="form-group row">
        ...
        <fieldset class="form-group row">
          ...
          <div class="col-sm-10">
            <div class="row @error('type') is-invalid @enderror">
              @foreach (\Arr::get($options, 'link.type') ?? [] as $option)
                <div class="col">
                  <div class="form-check">
                    <input class="form-check-input @error('type') is-invalid @enderror" type="radio" name="type"
                      value="{{ $option['value'] }}" @if ($link->type == $option['value']) checked @endif>
        ...
        <fieldset class="form-group row">
          ...
          <div class="col-sm-10">
            <div class="row @error('status') is-invalid @enderror">
              @foreach (\Arr::get($options, 'link.status') ?? [] as $option)
                <div class="col">
                  <div class="form-check">
                    <input class="form-check-input @error('status') is-invalid @enderror" type="radio" name="status"
                      value="{{ $option['value'] }}" @if ($link->status == $option['value']) checked @endif>
                    <label class="form-check-label">
                      {{ $option['name'] }}
                    </label>
                  </div>
        ...
        <div class="form-group row">
          <label class="col-sm-2 col-form-label">Keywors</label>
          <div class="col-sm-10">
            <input type="text" class="form-control" name="keywords" value="{{ $link->keywords }}">
          </div>
        </div>
        <div class="form-group row">
            ...
            <select class="form-control" name="parent">
              ...
              @isset($links)
                @foreach ($links as $_link)
                  <option value="{{ $_link->id }}" class="d-flex align-items-center"
                    @if ($_link->id == $link->parent) selected @endif>
                    {{ $_link->status }} |
                    {{ $_link->type }} |
                    {{ $_link->title }}
                  </option>
      ...
      <div class="card-footer d-flex justify-content-end">
        <a type="submit" class="btn btn-danger mr-auto"
          href="{{ url((isset($attributes) ? $attributes['alias'] . '/' : 'home/') . 'delete-link/' . $link->id) }}"
          onclick="event.preventDefault();document.getElementById('destroy-form').submit();">destroy</a>
        <a href="{{ url(isset($attributes) ? $attributes['alias'] . '/' : 'home/') }}" role="button"
          class="btn ml-2 btn-secondary" data-dismiss="modal">Cancel</a>
    ...
    <form id="destroy-form"
      action="{{ url((isset($attributes) ? $attributes['alias'] . '/' : 'home/') . 'delete-link/' . $link->id) }}"
