...
@php
  $plugins = json_decode(File::get(public_path('plugins.json')), true);

  // dump($plugins);
  // dump(explode(':', $slug));
  [$name, $version, $filename] = array_merge(explode(':', $slug), [null, null]);
  // dump($slug);
  $version = $version ?? 'latest';
  $config = $plugins[$name][$version];
  $script = $config['main']['script'];
  if ($script) {
      $path = "public/plugins/{$name}/{$version}/$script";
      $path = str_replace(['{$name}', '{$version}'], [$name, $version], $path);
  }
  // dump($path);
...
@empty($path ?? $src)
  <noscript type="text/javascript" src="{{ $path ? asset($path) : $src }}" data-plugin="{{ $name }}" data-src="{{ $src }}"></noscript>
@else
  <script type="text/javascript" src="{{ $path ? asset($path) : $src }}" data-plugin="{{ $name }}" data-src="{{ $src }}"></script>
@endempty
