...
@php
  ...
  // dd($config['main']);
  $style = Arr::get($config, 'main.style');
  // dd($style);
  if ($style) {
      $path = "public/plugins/{$name}/{$version}/$style";
      $path = str_replace(['{$name}', '{$version}'], [$name, $version], $path);
  }
  // dd($path);
  // dd([$name, $version, $filename, $config, $path]);
  // dump($path);
...
@empty($path ?? $src)
@else
  <link rel="stylesheet" href="{{ $path ? asset($path) : $src }}" data-plugin="{{ $name }}" data-href="{{ $src }}">
@endempty
