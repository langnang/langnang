    ...
    <el-col :span="24">
      <el-form label-width="80px" size="mini" ref="form">
        ...
        <el-form-item label="系列">
          <el-radio-group v-model="catlist.activeCat" @change="handleChangeCat" v-loading="catlist.loading">
            <el-radio-button :label="-1">{{
              catlist.all.name
            }}</el-radio-button>
            <el-radio-button v-for="(cat, key) in catlist.categories" :key="key" :label="key">{{ cat }}</el-radio-button>
          </el-radio-group>
        </el-form-item>
        <el-form-item label="分类" v-if="catlist.activeCat != '-1'">
          <el-radio-group v-model="catlist.activeSub" @change="handleChangeSub">
            <el-radio-button v-for="(cat, key) in catlist.sub.filter(
              item => item.category == catlist.activeCat
            )" :key="key" :label="cat.name"></el-radio-button>
          </el-radio-group>
        </el-form-item>
        <el-form-item label="歌单" v-if="playlists.tableData.length > 0">
          <el-scrollbar style="height:140px">
            <el-radio-group v-model="playlists.active" @change="handleChangePlaylist" v-infinite-scroll="load">
              <el-radio-button v-for="(play, key) in playlists.tableData" :key="key" :label="play.id">{{ play.name }}</el-radio-button>
            </el-radio-group>
          </el-scrollbar>
        </el-form-item>
        <el-form-item>
          <el-button :disabled="playlist.tableData.length == 0" @click="handlePlay" type="info">播放</el-button>
          <el-button :disabled="playlist.tableData.length == 0" @click="handleAddtoPlaylist" type="info">添加至播放列表</el-button>
        </el-form-item>
      </el-form>
    </el-col>
    <el-col :span="24">
...
<script>
import {
  get_netmusic_playlist,
  get_netmusic_playlist_catlist,
  get_netmusic_playlist_top,
  get_netmusic_song,
  get_netmusic_song_url
} from "@/api/netmusic";
...
export default {
  ...
  methods: {
    ...
    getPlaylistCatlist() {
      ...
      get_netmusic_playlist_catlist().then(res => {
        if (res == "") {
    ...
    getTopPlaylist() {
      ...
      get_netmusic_playlist_top({ cat, pageNum: this.playlists.pageNum }).then(res => {
        this.playlists.tableData = [
          ...this.playlists.tableData,
          ...res.playlists
        ];
