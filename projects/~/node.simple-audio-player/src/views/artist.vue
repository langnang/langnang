      ...
      <el-form label-width="80px" size="mini" ref="form">
        ...
        <el-form-item label="歌手" v-if="artists.data.length > 0">
          <el-scrollbar style="height:112px">
            <el-radio-group v-model="artists.active" @change="handleChangeArtist" v-infinite-scroll="load">
              <el-radio-button v-for="ar in artists.data" :key="ar.id" :label="ar.id">{{ ar.name }}</el-radio-button>
            </el-radio-group>
          </el-scrollbar>
        </el-form-item>
        <el-form-item>
...
<script>
import {
  get_netmusic_toplist,
  get_netmusic_playlist,
  get_netmusic_artist_list,
  get_netmusic_artist_top_songs,
...
export default {
  ...
  methods: {
    ...
    getArtistTopSongs() {
      ...
      get_netmusic_artist_top_songs(id).then(res => {
        get_netmusic_song_url(res.songs).then(r => {
